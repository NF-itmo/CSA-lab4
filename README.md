...
- Строки: один символ хранится в одном машинном слове, обработка выполняется программными процедурами
- Data stack хранится в `DataMemory`. `A0/DSP` указывает на текущую вершину стека: push сначала делает `A0++`, затем пишет в `DataMem[A0]`; pop читает `DataMem[A0]`, затем делает `A0--`
...
|--------------------------|--------:|---|
| `CODE_WORD_SIZE`         | 5 байт  | Окно выборки из памяти команд, память команд адремуется побайтово |
| `DATA_WORD_SIZE`         | 4 байта | Машинное слово данных, значение `int32` |
...
Вход внешнего устройства моделируется одним pending-слотом. Если к моменту нового события старое значение ещё не было прочитано через `CS` в режиме чтения, старое значение безвозвратно перезаписывается новым; `irq` остаётся поднятым для последнего доступного значения. Автоматический `0`-терминатор входного потока планируется как следующий символ после последнего заданного события с тем же минимальным интервалом, который указан во входном расписании; для одиночного события используется интервал `1000` тактов.
...
| 1   : any data                  |
|    ...                          |
| D-1 : empty data stack sentinel | <- A0 init
| D   : data stack first cell     |
|    ...                          |
...
`Code memory` адресуется побайтно. В её начале лежат две инструкции инициализации (`MOV A0,IMM D-1`, `MOV A1,IMM R`), затем `JMP` на старт программы, затем таблица векторов прерываний (по `INTERRUT_SIZE=3` байта на вектор), затем основной код

`Data memory` адресуется по словам. Одно слово данных занимает `4` байта (32 бита). До запуска транслятор кладёт в начало памяти литералы, константы, переменные, `allot`-блоки и null-terminated строки в порядке их аллокации. После статической области начинается data stack: адрес первой стековой ячейки `D`, а начальное значение пустого стека `D-1` загружается в `A0`. Если статическая область пуста, транслятор берёт `D=1`, чтобы sentinel-адрес оставался представимым неотрицательным числом. Return stack размещается отдельным диапазоном: его стартовый адрес `R` вычисляется транслятором и загружается в `A1`. Оба стека растут вверх.

Data stack представлен в памяти: `A0` указывает на текущую вершину стека. Режим `+(ST)` сначала увеличивает `A0`, затем пишет в новую вершину; режим `(ST)-` читает текущую вершину и затем уменьшает `A0`; режим `(ST)` обращается к `A0` без изменения глубины
...
- Строковые литералы (`cstr`): размещаются в `Data memory` как последовательность кодов символов и завершающий `0`
...
Оба кэша работают с байтовыми адресами внутри cache-line. Адрес, который приходит от CPU, сначала переводится в `byte_addr`:

| Кэш | CPU address | `addr_scale` | `byte_addr` |
|---|---|---:|---|
| `CodeCache` | байтовый `PC` | `1` | `PC` |
| `DataCache` | адрес машинного слова данных | `DATA_WORD_SIZE = 4` | `addr * 4` |

Дальше `byte_addr` раскладывается на битовые поля:
...
То есть:
- `offset` выбирает байт внутри 16-байтовой cache-line (`0..15`)
- `set_idx` выбирает один из 4 наборов (`0..3`)
- `tag` хранится в строке кэша и отличает разные memory-line, попавшие в один набор
- `way` не кодируется в адресе: при hit кэш ищет совпадающий `tag` среди 4 way выбранного набора
...
| test            | variant            | line size | lines | ways | cap  | ticks  | total HR | total acc | total hits | total miss | code HR | data HR |
|-----------------|--------------------|-----------|-------|------|------|--------|----------|-----------|------------|------------|---------|---------|
| cat             | 4way_line32_cap256 | 32        | 8     | 4    | 256  | 120145 | 99.98%   | 40988     | 40979      | 9          | 99.97%  | 99.98%  |
| cat             | 2way_line16_cap256 | 16        | 16    | 2    | 256  | 120143 | 99.96%   | 40960     | 40943      | 17         | 99.94%  | 99.97%  |
| cat             | 4way_line16_cap256 | 16        | 16    | 4    | 256  | 120143 | 99.96%   | 40960     | 40943      | 17         | 99.94%  | 99.97%  |
| cat             | 2way_line32_cap256 | 32        | 8     | 2    | 256  | 120145 | 99.98%   | 40988     | 40979      | 9          | 99.97%  | 99.98%  |
| |
| double          | 4way_line32_cap256 | 32        | 8     | 4    | 256  | 8097   | 97.51%   | 2690      | 2623       | 67         | 93.61%  | 99.55%  |
| double          | 2way_line16_cap256 | 16        | 16    | 2    | 256  | 8367   | 96.51%   | 2690      | 2596       | 94         | 91.23%  | 99.26%  |
| double          | 4way_line16_cap256 | 16        | 16    | 4    | 256  | 8477   | 96.10%   | 2690      | 2585       | 105        | 90.04%  | 99.26%  |
| double          | 2way_line32_cap256 | 32        | 8     | 2    | 256  | 8027   | 97.77%   | 2690      | 2630       | 60         | 94.37%  | 99.55%  |
| |
| hello           | 4way_line32_cap256 | 32        | 8     | 4    | 256  | 1525   | 97.30%   | 481       | 468        | 13         | 97.49%  | 97.16%  |
| hello           | 2way_line16_cap256 | 16        | 16    | 2    | 256  | 1575   | 96.26%   | 481       | 463        | 18         | 95.98%  | 96.45%  |
| hello           | 4way_line16_cap256 | 16        | 16    | 4    | 256  | 1575   | 96.26%   | 481       | 463        | 18         | 95.98%  | 96.45%  |
| hello           | 2way_line32_cap256 | 32        | 8     | 2    | 256  | 1525   | 97.30%   | 481       | 468        | 13         | 97.49%  | 97.16%  |
| |
| hello_user_name | 4way_line32_cap256 | 32        | 8     | 4    | 256  | 151892 | 99.94%   | 51784     | 51752      | 32         | 99.93%  | 99.94%  |
| hello_user_name | 2way_line16_cap256 | 16        | 16    | 2    | 256  | 151983 | 99.85%   | 51658     | 51580      | 78         | 99.81%  | 99.88%  |
| hello_user_name | 4way_line16_cap256 | 16        | 16    | 4    | 256  | 151929 | 99.91%   | 51742     | 51694      | 48         | 99.90%  | 99.92%  |
| hello_user_name | 2way_line32_cap256 | 32        | 8     | 2    | 256  | 151892 | 99.86%   | 51644     | 51571      | 73         | 99.85%  | 99.86%  |
| |
| poly            | 4way_line32_cap256 | 32        | 8     | 4    | 256  | 175    | 82.86%   | 35        | 29         | 6          | 82.35%  | 83.33%  |
| poly            | 2way_line16_cap256 | 16        | 16    | 2    | 256  | 225    | 68.57%   | 35        | 24         | 11         | 64.71%  | 72.22%  |
| poly            | 4way_line16_cap256 | 16        | 16    | 4    | 256  | 225    | 68.57%   | 35        | 24         | 11         | 64.71%  | 72.22%  |
| poly            | 2way_line32_cap256 | 32        | 8     | 2    | 256  | 175    | 82.86%   | 35        | 29         | 6          | 82.35%  | 83.33%  |
| |
| problem1        | 4way_line32_cap256 | 32        | 8     | 4    | 256  | 231851 | 97.25%   | 75712     | 73629      | 2083       | 91.78%  | 99.98%  |
| problem1        | 2way_line16_cap256 | 16        | 16    | 2    | 256  | 247861 | 95.13%   | 75712     | 72028      | 3684       | 85.45%  | 99.96%  |
| problem1        | 4way_line16_cap256 | 16        | 16    | 4    | 256  | 247841 | 95.14%   | 75712     | 72030      | 3682       | 85.46%  | 99.96%  |
| problem1        | 2way_line32_cap256 | 32        | 8     | 2    | 256  | 231841 | 97.25%   | 75712     | 73630      | 2082       | 91.78%  | 99.98%  |
| |
| sort            | 4way_line32_cap256 | 32        | 8     | 4    | 256  | 140372 | 99.79%   | 47798     | 47696      | 102        | 99.55%  | 99.96%  |
| sort            | 2way_line16_cap256 | 16        | 16    | 2    | 256  | 141364 | 99.51%   | 47686     | 47452      | 234        | 98.93%  | 99.93%  |
| sort            | 4way_line16_cap256 | 16        | 16    | 4    | 256  | 140910 | 99.66%   | 47770     | 47606      | 164        | 99.28%  | 99.93%  |
| sort            | 2way_line32_cap256 | 32        | 8     | 2    | 256  | 141139 | 99.60%   | 47756     | 47565      | 191        | 99.11%  | 99.96%  |
...
2. Синтаксический проход: разбор объявлений (`variable`, `constant`, `allot`), определений слов и `interruption_n`
3. Генерация кода: стековые слова и управляющие конструкции переводятся в ISA (с patch-адресацией для переходов)
4. Построение памяти:
...
5. Сериализация в настоящие бинарные файлы (`binary` вариант)

Пайплайн трансляции:
```text
.fth
  -> tokenize
  -> parse declarations/words/interruption_n
...
- Все строки компилируются как `cstr` в `DataMemory`: один символ на машинное слово и завершающий `0`
...
- Каждый `GLOBAL_NETLIST.tick()` - один такт (распространение сигналов + тактирование компонентов).
- CU выполняет инструкцию как набор явных микроопераций (изменения сигналов и защёлок).
- При кэш-промахах память поднимает `busy`, а CU ждёт завершения подгрузки линий.
- Логгер фиксирует состояние на каждом такте, включая текущую инструкцию (`instruction:mnemonic/hex`) и признак режима прерывания (`DI`).

## Control Unit

Основная модель находится в `src/Machine.py` и использует `CU2`. CU не вызывает такты сам: он хранит текущую инструкцию в `IR`, а номер текущего микрошагa - в `STEP_COUNTER`. На каждом внешнем `GLOBAL_NETLIST.tick()` CU только выставляет управляющие сигналы: защёлки регистров, селекторы mux-ов, режимы ALU/памяти/устройств и константу для `PCAdder`

### Фазы исполнения инструкции

Один внешний такт симуляции - это один вызов `GLOBAL_NETLIST.tick()`: сначала схема стабилизируется, затем тактируются компоненты, затем схема снова стабилизируется. `CU2` не продвигает время сам, а только выставляет управляющие сигналы для текущей фазы.
...
| `double.yaml` | Арифметика двойной точности средствами языка | Десятичный вывод результата |
