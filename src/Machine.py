...
class Netlist:
    ...
    def _settle(self, limit: int = 20) -> None:
        ...
        for _ in range(limit):
            ...
            # propagate wires
            old_values = [(wire.src.value, wire.dst.value) for wire in self.wires]
        ...
        raise RuntimeError("Netlist did not stabilize (possible combinational loop)")
...
class MuxN(Component):
    ...
    def __init__(self, name: str, n: int = 2) -> None:
        ...
        self.inputs = [Socket(self, f"input{i}", is_input=True) for i in range(n)]
...
class CodeMemory(Component):
    ...
    def tick(self) -> None:
        ...
        self.output.set(
            int.from_bytes(raw.ljust(CODE_WORD_SIZE, b"\x00"), byteorder="big")
        )
...
class Mask(Component):
    ...
    def tick(self) -> None:
        self.output.set(self.input.value & self._mask)
...
class Adder(Component):
    ...
    def tick(self) -> None:
        self.output.set(self.input.value + self.add_input.value)
...
class AdressDecoder(Component):
    def __init__(self, n: int):
        ...
        self.cs_outputs = [
            Socket(self, f"cs_output{i}", is_output=True) for i in range(n)
        ]
...
class InstructionLogFormatter:
    ...
    def __call__(self, addr: int, instruction: int, instr_size: int) -> str:
        ...
        hex_view = (
            (instruction >> (8 * (CODE_WORD_SIZE - instr_size)))
            .to_bytes(
                instr_size,
                byteorder="big",
            )
...
class CU(Component):
    ...
    def __init__(self) -> None:
        ...
        self.external_output_mux = self._create_component(
            MuxN, "ExternalOutputMux", INTERRUPT_COUNT
        )
        self.external_irq_mux = self._create_component(
            MuxN, "ExternalIrqMux", INTERRUPT_COUNT
        )
        self.external_irq_or = self._create_component(
            OrN, "ExternalIrqOr", INTERRUPT_COUNT
        )
        self.irq = Socket(self, "irq", is_input=True)
        self.irq_device = Socket(self, "irq_device", is_input=True)
        self.interrupt_vector_base = instruction_size(Opcode.LD) * 2 + instruction_size(
            Opcode.JMP
        )
        ...
        self.a1 = self._create_component(
            Register, "A1"
        )  # aka RSP - Return Stack Pointer
        ...
        self.vector_target_mask = self._create_component(
            ExtractBits, "CodeVectorTarget", 0xFFFFFFFF, 8
        )
    ...
    def _create_component[T: Component](
        self, component: Type[T], *args: ..., **kwargs: ...
    ) -> T:
    ...
    def _tick(self) -> None:
        ...
        print(f"0x{GLOBAL_NETLIST.tick_counter:06x}", f"[{interrupt_mode}]", end=": ")
        print(
            self._current_instruction_log,
        )
        print(
    ...
    def run(self, limit: int = 10_000) -> None:
        ...
        for _ in range(limit):
            ...
            for arg_size in Args[opcode.name].value:
                arg_bits_left -= arg_size * 8
                args.append(
                    (instruction >> arg_bits_left) & ((1 << (arg_size * 8)) - 1)
                )
            ...
            if (
                ...
                for device_idx in range(INTERRUPT_COUNT):
                    self.external_irq_mux.sel.set(
                        device_idx
                    )  # ExternalIrqMux выбирает IRQ проверяемого устройства
                ...
                if irq_device is not None:
                    ...
                    self.device_decoder.addr.set(
                        irq_device
                    )  # AD получает номер устройства с IRQ
                    self.device_decoder.cs_input.set(1)  # AD выдаёт CS устройству с IRQ
                    self.external_devices[irq_device].is_write.set(
                        0
                    )  # ExternalDevice переводится в режим чтения
                    self.external_output_mux.sel.set(
                        irq_device
                    )  # ExternalOutputMux выбирает устройство с IRQ
                    self._tick()  # Такт - ExternalDevice выдаёт данные IRQ
                    self.device_decoder.cs_input.set(
                        0
                    )  # AD закрывает CS внешних устройств

                    # Сохраняем данные IRQ в D0
                    self.write_to_register_mux.sel.set(
                        3
                    )  # WriteToReg выбирает выход ExternalDevice
                    ...
                    self.alu.operation.set(
                        self.alu.OP_PASS_A
                    )  # ALU пропускает D0 на вход Data Memory
                    self.reg_to_data_mem_mux.sel.set(
                        3
                    )  # RegMuxAddr выбирает DSP как адрес Data Memory
                    self.data_mem_addr_mux.sel.set(
                        1
                    )  # DataMemAddrMux выбирает адрес из DSP
                    self.data_mem_input_mux.sel.set(0)  # DataMemInputMux выбирает ALU
                    self.data_mem.is_write.set(
                        1
                    )  # Data Memory переводится в режим записи
                    ...
                    self.data_mem.is_write.set(
                        0
                    )  # Data Memory возвращается в режим чтения

                    # Сохраняем текущий PC и увеличиваем DSP
                    self.alu_left_mux.sel.set(3)  # Левый вход ALU выбирает DSP
                    self.alu.operation.set(
                        self.alu.OP_INC
                    )  # ALU переходит в режим инкремента DSP
                    self.write_to_register_mux.sel.set(
                        0
                    )  # WriteToReg выбирает результат ALU
                    self.a0.latch.set(0)  # Снимаем защёлку с DSP
                    self.pc_adder.add_input.set(
                        0
                    )  # На ADD подаётся 0 для сохранения текущего PC
                    self.reg_to_data_mem_mux.sel.set(
                        4
                    )  # RegMuxAddr выбирает RSP как адрес Data Memory
                    self.data_mem_addr_mux.sel.set(
                        1
                    )  # DataMemAddrMux выбирает адрес из RSP
                    self.data_mem_input_mux.sel.set(
                        1
                    )  # DataMemInputMux выбирает текущий PC
                    self.data_mem.is_write.set(
                        1
                    )  # Data Memory переводится в режим записи
                    self.data_mem.latch.set(1)  # Data Memory открывает latch
                    ...
                    self.alu.operation.set(
                        self.alu.OP_INC
                    )  # ALU переходит в режим инкремента RSP
                    self.write_to_register_mux.sel.set(
                        0
                    )  # WriteToReg выбирает результат ALU
                    ...
                    self.pc_mux.inputs[4].set(
                        self.interrupt_vector_base + irq_device * 4
                    )  # CU вычисляет адрес вектора
                    ...
                    self.pc_mux.sel.set(
                        5
                    )  # PCMux выбирает адрес обработчика из вектора
                    self.pc.latch.set(0)  # Снимаем защёлку с PC
            ...
            if opcode == Opcode.NOP:
                ...
                self.pc_adder.add_input.set(
                    instr_size
                )  # На ADD подаётся размер текущей инструкции
            ...
            if opcode == Opcode.LD:
                ...
                self.write_to_register_mux.sel.set(
                    2
                )  # WriteToReg выбирает immediate из Code Memory
            ...
            if opcode == Opcode.CALL:
                ...
                self.data_mem_input_mux.sel.set(
                    1
                )  # DataMemInputMux выбирает PC + instr_size
            ...
            if opcode == Opcode.RET or opcode == Opcode.IRET:
                ...
                self.alu.operation.set(
                    self.alu.OP_DEC
                )  # ALU переходит в режим декремента RSP
                self.write_to_register_mux.sel.set(
                    0
                )  # WriteToReg выбирает результат ALU
                self.a1.latch.set(0)  # Снимаем защёлку с RSP
            ...
            if opcode == Opcode.INT:
                ...
                self.alu.operation.set(
                    self.alu.OP_DEC
                )  # ALU переходит в режим декремента DSP
                self.write_to_register_mux.sel.set(
                    0
                )  # WriteToReg выбирает результат ALU
                self.a0.latch.set(0)  # Снимаем защёлку с DSP
                ...
                self.write_to_register_mux.sel.set(
                    1
                )  # WriteToReg выбирает выход Data Memory
                ...
                self.pc_mux.inputs[4].set(
                    self.interrupt_vector_base + int_number * 4
                )  # CU вычисляет адрес вектора
            ...
            if opcode == Opcode.SCALL:
                ...
                self.data_mem_addr_mux.sel.set(
                    1
                )  # DataMemAddrMux выбирает адрес из регистра
                self.data_mem.is_write.set(0)  # Data Memory переводится в режим чтения
                self.data_mem.latch.set(1)  # Data Memory открывает latch
                self.write_to_register_mux.sel.set(
                    1
                ...
                self.alu.operation.set(
                    self.alu.OP_PASS_A
                )  # ALU пропускает execution token без изменения
            ...
            if opcode == Opcode.PUSH:
                ...
                if addr_mode == AddressingMode.IMM:
                    ...
                    self.data_mem_addr_mux.sel.set(
                        0
                    )  # DataMemAddrMux выбирает адрес из инструкции
                    self.data_mem.is_write.set(
                        0
                    )  # Data Memory переводится в режим чтения
                    self.data_mem.latch.set(1)  # Data Memory открывает latch
                    self.write_to_register_mux.sel.set(
                        1
                    )  # WriteToReg выбирает выход Data Memory
                    ...
                    self.reg_to_data_mem_mux.sel.set(
                        1
                    )  # RegMuxAddr выбирает D1 как адрес Data Memory
                    self.data_mem_addr_mux.sel.set(
                        1
                    )  # DataMemAddrMux выбирает адрес из регистра
                    self.data_mem.is_write.set(
                    ...
                    self.alu_left_mux.sel.set(
                        args[1]
                    )  # Левый вход ALU выбирает регистр-источник
                    self.alu.operation.set(
                        self.alu.OP_PASS_A
                    )  # ALU пропускает выбранный регистр
                    self.write_to_register_mux.sel.set(
                        0
                    )  # WriteToReg выбирает результат ALU
                    self.d0.latch.set(0)  # Снимаем защёлку с D0
            ...
            if opcode == Opcode.POP:
                ...
                if addr_mode == AddressingMode.STI:
                    ...
                    self.data_mem_addr_mux.sel.set(
                        1
                    )  # DataMemAddrMux выбирает адрес из D1
                    self.data_mem.is_write.set(
                        1
                    )  # Data Memory переводится в режим записи
                    self.data_mem.latch.set(1)  # Data Memory открывает latch
            ...
            if opcode in {Opcode.SPLS, Opcode.SMIN, Opcode.SMUL, Opcode.SDIV}:
                ...
                if opcode == Opcode.SPLS:
                    self.alu.operation.set(
                        self.alu.OP_ADD
                    )  # ALU переходит в режим сложения
                elif opcode == Opcode.SMIN:
                    self.alu.operation.set(
                        self.alu.OP_SUB
                    )  # ALU переходит в режим вычитания
                elif opcode == Opcode.SMUL:
                    self.alu.operation.set(
                        self.alu.OP_MUL
                    )  # ALU переходит в режим умножения
                else:
                    self.alu.operation.set(
                        self.alu.OP_DIV
                    )  # ALU переходит в режим деления
            ...
            if opcode in {Opcode.EQ, Opcode.GT, Opcode.LT}:
                ...
                if opcode == Opcode.EQ:
                    self.alu.operation.set(
                        self.alu.OP_EQ
                    )  # ALU переходит в режим сравнения на равенство
                elif opcode == Opcode.GT:
                    self.alu.operation.set(
                        self.alu.OP_GT
                    )  # ALU переходит в режим сравнения greater-than
                else:
                    self.alu.operation.set(
                        self.alu.OP_LT
                    )  # ALU переходит в режим сравнения less-than
            ...
            if opcode == Opcode.SWAP:
                ...
                self.alu.operation.set(
                    self.alu.OP_PASS_A
                )  # ALU пропускает D1 на вход Data Memory
                self.reg_to_data_mem_mux.sel.set(
                    3
                )  # RegMuxAddr выбирает DSP как адрес Data Memory
                self.data_mem_addr_mux.sel.set(
            ...
            if opcode == Opcode.JZ or opcode == Opcode.JNZ:
                ...
                do_jump = (predicate == 0 and opcode == Opcode.JZ) or (
                    predicate != 0 and opcode == Opcode.JNZ
                )
                if do_jump:
                    # Загружаем PC адресом перехода
                    self.pc_mux.sel.set(
                        0
                    )  # PCMux выбирает адрес из аргумента инструкции
                    self.pc.latch.set(0)  # Снимаем защёлку с PC
                    self._tick()  # Такт - PC получает адрес перехода
                    self.pc.latch.set(1)  # Защёлкиваем PC
                else:
            ...
            if opcode == Opcode.IN:
                ...
                self.device_decoder.addr.set(
                    device_idx
                )  # AD получает номер внешнего устройства
                self.device_decoder.cs_input.set(
                    1
                )  # AD выдаёт CS выбранному устройству
                self.external_devices[device_idx].is_write.set(
                    0
                )  # ExternalDevice переводится в режим чтения
                self.external_output_mux.sel.set(
                    device_idx
                )  # ExternalOutputMux выбирает выбранное устройство
            ...
            if opcode == Opcode.OUT:
                ...
                self.reg_to_data_mem_mux.sel.set(
                    output_value_sel
                )  # RegMuxAddr выбирает значение для ExternalDevice
                self.device_decoder.addr.set(
                    device_idx
                )  # AD получает номер внешнего устройства
                self.device_decoder.cs_input.set(
                ...
                self.external_devices[device_idx].is_write.set(
                    1
                )  # ExternalDevice переводится в режим записи
                self._tick()  # Такт - ExternalDevice принимает значение
                self.device_decoder.cs_input.set(0)  # AD закрывает CS внешних устройств
                self.external_devices[device_idx].is_write.set(
                    0
                )  # ExternalDevice возвращается в режим чтения
            ...
            if opcode == Opcode.INC or opcode == Opcode.DEC:
                ...
                if opcode == Opcode.INC:
                    self.alu.operation.set(
                        self.alu.OP_INC
                    )  # ALU переходит в режим инкремента
                else:
                    self.alu.operation.set(
                        self.alu.OP_DEC
                    )  # ALU переходит в режим декремента
                self.reg_to_data_mem_mux.sel.set(
                    3
                )  # RegMuxAddr выбирает DSP как адрес Data Memory
                self.data_mem_addr_mux.sel.set(
...
if __name__ == "__main__":
    ...
    print(read_file(code_path).hex())
    cu = CU()
    cu.load(read_file(data_path), read_file(code_path))
